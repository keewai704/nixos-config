{ config, lib, pkgs, ... }:
{
  ...
  systemd.services.tailscale-autoconnect =
    lib.mkIf (config.sops.secrets ? "tailscale/authkey") {
      ...
      after = [ "NetworkManager.service" "network-online.target" "tailscaled.service" "sops-nix.service" ];
      wants = [ "NetworkManager.service" "network-online.target" "tailscaled.service" "sops-nix.service" ];
